...
$clinicId    = (int)($user['id'] ?? 0);
$clinicName  = (string)($user['name'] ?? 'Clínica');
$clinicEmail = (string)($user['email'] ?? '');
...
// =====================
// CARGA OFERTAS (LÓGICA + DATA) SIN RENDERIZAR
// =====================
$PILLAR_OFFERS_RENDER = false;
require __DIR__ . "/ofertas.php"; // carga $jobs y $kpiActiveOffers y procesa POST cuando tab=ofertas
...
try {
  ...
  // Verifica tabla clinic_events
  $stT = $pdo->prepare("SHOW TABLES LIKE 'clinic_events'");
  $stT->execute();
  $hasClinicEvents = (bool)$stT->fetchColumn();
  if (!$hasClinicEvents) {
    throw new RuntimeException("No existe la tabla clinic_events.");
  }

  // Detecta columnas existentes
  $cols = [];
  $stC = $pdo->query("SHOW COLUMNS FROM clinic_events");
  foreach (($stC->fetchAll(PDO::FETCH_ASSOC) ?: []) as $r) {
    $cols[] = (string)$r['Field'];
  }

  $required = ['id', 'clinic_user_id', 'event_date', 'title'];
  foreach ($required as $req) {
    if (!in_array($req, $cols, true)) {
      throw new RuntimeException("clinic_events no tiene la columna requerida: {$req}");
    }
  }
  ...
  // Acciones POST (solo en calendario)
    ...
    if (in_array($action, ['create_event', 'update_event', 'delete_event'], true)) {
      ...
      if (!$errs) {
        ...
        if ($action === 'update_event') {
          ...
          $stOwn = $pdo->prepare("SELECT id FROM clinic_events WHERE id=? AND clinic_user_id=? LIMIT 1");
          $stOwn->execute([$eventId, $clinicId]);
          ...
          if (!$errs) {
            ...
            $sql = "UPDATE clinic_events SET " . implode(',', $sets) . " WHERE id=? AND clinic_user_id=? LIMIT 1";
            $st = $pdo->prepare($sql);
            $st->execute($vals);
        ...
        if ($action === 'delete_event') {
          $stDel = $pdo->prepare("DELETE FROM clinic_events WHERE id=? AND clinic_user_id=? LIMIT 1");
          $stDel->execute([$eventId, $clinicId]);
  ...
  $stM->execute([$clinicId, $rangeStart, $rangeEnd]);
  ...
  if ($tab === 'calendario' && $editId > 0) {
    $stE = $pdo->prepare("SELECT * FROM clinic_events WHERE id=? AND clinic_user_id=? LIMIT 1");
    $stE->execute([$editId, $clinicId]);
    $editing = $stE->fetch(PDO::FETCH_ASSOC) ?: null;
  }

} catch (Throwable $e) {
  error_log("CLINICA INDEX (calendar) ERROR: " . $e->getMessage());
  if ($tab === 'calendario') {
...
// =====================
// KPI: Aplicantes (count) — no rompe UI si falla
// =====================
try {
  if (isset($pdo) && ($pdo instanceof PDO)) {
    $st = $pdo->prepare("
      ...
      WHERE j.`clinic_user_id` = ?
    ");
    $st->execute([$clinicId]);
    $kpiApplicants = (int)$st->fetchColumn();
  }
} catch (Throwable $e) {
  error_log('KPI Applicants ERROR: ' . $e->getMessage());
}

// Perfil mock
$profile = [
  'name' => $clinicName ?: 'Clínica',
...
$pageTitle = ($plLang === 'en') ? 'Clinic Dashboard | PILLAR Locums' : 'Panel Clínica | PILLAR Locums';
?>
<!doctype html>
<html lang="<?= h($plLang) ?>">
<head>
...
<body>
  ...
  <main class="container">
    <div class="hero">
      <div>
        <h1><?= h($profile['name']) ?> — <?= h(pl_t('clinic_panel')) ?></h1>
        <p><?= h($profile['location']) ?> • <?= h($profile['type']) ?></p>
        <p class="muted" style="margin-top:8px;">
          <?= h(pl_t('session')) ?>: <?= h($clinicEmail) ?> • ID: <?= (int)$clinicId ?>
        </p>
      </div>
    </div>
        ...
        <?php elseif ($tab === 'calendario'): ?>
          ...
          <div class="grid2">
            <div class="panel">
              ...
              <?php if (!empty($job) && (($job['status'] ?? '') === 'aceptado')): ?>
                <div class="contacto">
                  <h3><?= h(pl_t('assigned_doctor')) ?></h3>
                  <p><?= h((string)($job['name'] ?? '')) ?></p>
                  <p><?= h((string)($job['phone'] ?? '')) ?></p>

                  <a href="/mensajes/chat.php?job=<?= (int)($job['id'] ?? 0) ?>">
                    <?= h(pl_t('open_chat')) ?>
                  </a>
                </div>
              <?php endif; ?>
